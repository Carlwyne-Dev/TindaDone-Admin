...
module.exports = async (req, res) => {
  ...
  if (password !== (process.env.ADMIN_PASSWORD || 'xyuuki18')) return res.status(401).json({ error: 'Auth Failed' });

  const { url: KV_URL, token: KV_TOKEN } = getKVEnv();
  if (!KV_URL || !KV_TOKEN) return res.status(200).json({ logs: [], info: 'DB Not Linked' });

  try {
    const data = await kvRequest(`${KV_URL}/lrange/recent_trials/0/50`, {
