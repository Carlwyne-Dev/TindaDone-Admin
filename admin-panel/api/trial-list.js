...
export default async function handler(req, res) {
  ...
  if (password !== (process.env.ADMIN_PASSWORD || 'xyuuki18')) return res.status(401).json({ message: 'Unauthorized' });

  const { url: KV_URL, token: KV_TOKEN } = getKVEnv();
  if (!KV_URL || !KV_TOKEN) return res.status(200).json({ logs: [] });

  try {
    const response = await fetch(`${KV_URL}/lrange/recent_trials/0/50`, { headers: { Authorization: `Bearer ${KV_TOKEN}` } });
